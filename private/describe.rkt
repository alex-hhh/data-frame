...
(define (df-describe df)
  ...
  (printf "data-frame: ~a columns, ~a rows, ~a indexes~%"
          (length (df-series-names df))
          (df-row-count df)
          (length (df-index-names df)))
  (printf "properties:~%")
  (let ((prop-names (df-property-names df))
    ...
    (for ([pn prop-names])
      ...
      (newline)))
  (let ([index-names (sort (df-index-names df) string<?)])
    (unless (null? index-names)
      (printf "indexes:~%")
      (for ([in (in-list index-names)])
        (printf "  ~a ON ~a~%" in (df-index-series df in)))))
  (printf "series:~%")
  (let ((series-names (sort (df-series-names df) string<?))
        (maxw 0))
    (for ([sn (in-list series-names)])
